 ...
 */
export function saveReport(report: ScenarioReport): string {
  ...
  for (let i = MAX_REPORTS_PER_SCENARIO; i < allForScenario.length; i++) {
    try { fs.unlinkSync(path.join(dir, allForScenario[i])); } catch { /* ignore */ }
  }
...
export function findLatestReport(): string | undefined {
  const dir = getResultsDir();
...
export function findLatestReportForScenario(scenario: string): string | undefined {
  const dir = getResultsDir();
  if (!fs.existsSync(dir)) return undefined;
...
export function listReports(scenario?: string): string[] {
  const dir = getResultsDir();
