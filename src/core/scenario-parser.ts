...
import type { ConversationTurn, EnvironmentOverride, Scenario, Step } from './types.js';

const MAX_REF_DEPTH = 5;

export function parseScenario(filePath: string, env?: string): Scenario {
  let content: string;
  try {
  ...
  const resolvedRawSteps = scenario.steps.map((rawStep: unknown, i: number) =>
    resolveRef(rawStep, scenarioDir, 0, `step ${i + 1}`)
  );

  const steps = resolvedRawSteps.map((rawStep: unknown, i: number) => validateStep(rawStep, i, scenarioDir));

  // Parse environments
  let environments: Record<string, EnvironmentOverride> | undefined;
  if (scenario.environments && typeof scenario.environments === 'object') {
    environments = {};
    for (const [envName, envVal] of Object.entries(scenario.environments as Record<string, unknown>)) {
      ...
      if (typeof envObj.iterations === 'number') override.iterations = envObj.iterations;
      if (envObj.variables && typeof envObj.variables === 'object') {
        override.variables = envObj.variables as Record<string, string>;
      }
      if (envObj.steps && typeof envObj.steps === 'object') {
        override.steps = envObj.steps as Record<string, Partial<Step>>;
      }
...
function applyEnvironment(scenario: Scenario, envName: string): Scenario {
  const envOverride = scenario.environments?.[envName];
  if (!envOverride) {
    throw new Error(
      `Environment "${envName}" not found. Available: ${Object.keys(scenario.environments ?? {}).join(', ') || 'none'}`
    );
  }
 ...
 */
function resolveRef(rawStep: unknown, baseDir: string, depth: number, pos: string): unknown {
  if (!rawStep || typeof rawStep !== 'object') return rawStep;

  const step = rawStep as Record<string, unknown>;
  if (typeof step.$ref !== 'string') return rawStep;
  ...
  let refContent: unknown;
  try {
    refContent = yaml.load(fs.readFileSync(refPath, 'utf-8'));
  } catch (e) {
    throw new Error(`${pos}: Invalid YAML in $ref file ${step.$ref}: ${(e as Error).message}`);
  }
  ...
  return { ...(resolved as Record<string, unknown>), ...localOverrides };
}

function validateStep(raw: unknown, index: number, scenarioDir?: string): Step {
  if (!raw || typeof raw !== 'object') {
  ...
  if (Array.isArray(step.assertions) && scenarioDir) {
    for (const assertion of step.assertions as Array<Record<string, unknown>>) {
