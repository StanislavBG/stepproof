    ...
    switch (type) {
        ...
        case 'hallucination': {
            ...
            try {
                ...
                return fail(type, `Cannot create hallucination judge adapter: ${e.message}`);
            }
            ...
            try {
                ...
                return fail(type, `Hallucination judge API call failed: ${e.message}`);
            }
            const passed = score <= maxScore;
            return passed
                ? pass(type)
        ...
        case 'faithfulness': {
            ...
            try {
                ...
                return fail(type, `Cannot create faithfulness judge adapter: ${e.message}`);
            }
            ...
            try {
                ...
                return fail(type, `Faithfulness judge API call failed: ${e.message}`);
            }
            const passed = score >= threshold;
            return passed
                ? pass(type)
        ...
        case 'prompt_injection': {
            ...
            try {
                ...
                return fail(type, `Cannot create prompt injection judge adapter: ${e.message}`);
            }
            ...
            try {
                ...
                return fail(type, `Prompt injection judge API call failed: ${e.message}`);
            }
        ...
        case 'language': {
            ...
            try {
                ...
                return fail(type, `Cannot create language judge adapter: ${e.message}`);
            }
            ...
            try {
                ...
                return fail(type, `Language judge API call failed: ${e.message}`);
            }
        ...
        case 'coherence': {
            ...
            try {
                ...
                return fail(type, `Cannot create coherence judge adapter: ${e.message}`);
            }
            ...
            try {
                ...
                return fail(type, `Coherence judge API call failed: ${e.message}`);
            }
            const passed = score >= threshold;
            return passed
                ? pass(type)
        ...
        case 'custom': {
            ...
            try {
                ...
                return fail(type, `Failed to load custom assertion plugin ${assertion.plugin}: ${e.message}`);
            }
            try {
                ...
                return fail(type, `Custom assertion plugin threw: ${e.message}`);
            }
        }
        default: {
            return fail(type, `Unknown assertion type: "${type}".`);
        }
    }
}
/**
...
export function updateSnapshot(scenarioDir, stepId, output) {
    const snapshotDir = path.resolve(scenarioDir, '.stepproof', 'snapshots');
    const snapshotPath = path.join(snapshotDir, `${stepId}.txt`);
    fs.mkdirSync(path.dirname(snapshotPath), { recursive: true });
    fs.writeFileSync(snapshotPath, output, 'utf-8');
