 ...
 */
export async function runGenerate(scenarioPath, opts) {
    ...
    if (variableNames.length === 0) {
        throw new Error('No template variables ({{variable}}) found in scenario prompts. ' +
    ...
    try {
        ...
        throw new Error(`Failed to parse generated test inputs: ${e.message}\nRaw response:\n${response.text}`);
    }
...
function buildMetaPrompt(templates, variableNames, existingVars, count) {
